...
Known gaps: no movement/rotation broadcasting yet for player entities (they appear frozen in their spawn pose until this ships), no player-entity metadata beyond skin parts (e.g. sneaking/sprinting pose), and a player's own F5 view doesn't show their hat layer even though other players see it correctly (see above).
...
// it being current, not just the position an item was spawned at) and
// broadcasting Update_Entity_Position_p so clients stay in sync. Constants
// below approximate vanilla item-entity physics for plausible-looking motion --
// a gameplay-feel choice, not a wire-format detail, so not decompile-verified
// the way packet layouts are.

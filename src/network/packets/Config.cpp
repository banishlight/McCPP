...
void Acknowledge_Finish_Config_p::deserialize(std::vector<Byte> in_buff, PacketContext& cont) {
    ...
    // never "spawned" to them), so it has to happen here instead. MUST come
    // after Login_Play_p: that's the packet that tells the client its own
    // entity ID even exists client-side -- an entity-metadata packet naming
    // that ID sent any earlier targets an entity the client hasn't created
    // yet and is silently dropped (this was that exact bug: the real
    // per-player skin-parts bitmask from Client Information never actually
    // reached the client's own local player model, only whatever a freshly
    // constructed entity defaults its metadata to).
